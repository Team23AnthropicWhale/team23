...
//
// Storage is injected via configureKeyStorage() so this file runs on any
// platform — mobile uses expo-secure-store, backend uses its own store.
...
// ---------------------------------------------------------------------------
// Injectable storage — call configureKeyStorage() before any key operations
// ---------------------------------------------------------------------------
...
export function configureKeyStorage(storage: KeyStorage): void {
  _storage = storage;
}

function store(): KeyStorage {
  if (!_storage) throw new Error('Call configureKeyStorage() before using key operations.');
  return _storage;
}
...
// ---------------------------------------------------------------------------
// Key persistence — delegates to the injected KeyStorage
// ---------------------------------------------------------------------------
...
export async function storeKey(key: CryptoKey, pin: string): Promise<void> {
  ...
  rawBytes.fill(0);
  await store().setItem(`cpms_key_${pin}`, b64);
}
...
export async function loadKey(pin: string): Promise<CryptoKey | null> {
  const b64 = await store().getItem(`cpms_key_${pin}`);
  if (!b64) return null;
...
export async function generateOrgKey(): Promise<string> {
  ...
  await store().setItem(ORG_KEY_STORE, b64);
  return b64;
}
...
export async function getOrgKey(): Promise<CryptoKey | null> {
  const b64 = await store().getItem(ORG_KEY_STORE);
  if (!b64) return null;
  return getSubtle().importKey(
    'raw',
    fromBase64(b64),
...
export async function exportOrgKeyForTransfer(sharedSecret: string): Promise<string> {
  const subtle     = getSubtle();
  const orgKeyB64  = await store().getItem(ORG_KEY_STORE);
